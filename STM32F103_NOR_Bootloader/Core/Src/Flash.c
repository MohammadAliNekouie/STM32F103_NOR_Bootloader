...
void FEE_Erase (void) {
	...
	do {
		FLASH_ErasePage(Flash_lower_half_start + (page_num * FLASH_PAGE_SIZE));
		page_num++;
	} while ((Flash_lower_half_start + (page_num * FLASH_PAGE_SIZE)) < Flash_lower_half_end);
}
...
void FLASH_Erase (uint32_t start_address,uint32_t end_address) {
	...
	// delete all pages from specified start page to the last page
	do {
		FLASH_ErasePage(start_address + (page_num * FLASH_PAGE_SIZE));
		page_num++;
	} while ((start_address + (page_num * FLASH_PAGE_SIZE)) < end_address);
}
/*****************************************************************************
...
uint16_t FEE_WriteMultiple(uint32_t Address,uint8_t * Value,uint16_t len) {
	...
	int i;
		...
		for(i = 0; i < len; i=i+2) 
		{
				FlashStatus = FLASH_ProgramHalfWord(Address + i , (uint16_t)((Value[i+1]<<8) | Value[i]));
...
{
	uint16_t page_num=((source_Page_end_address-source_Page_start_address)+1)/FLASH_PAGE_SIZE;
	...
	{
		FEE_ReadMultiple(read_address,DataBuf,FLASH_PAGE_SIZE);
		FEE_WriteMultiple(write_address,DataBuf,FLASH_PAGE_SIZE);
...
//all pages must erased before run this function and Flash must be unlocked
void Flash_Page_write(uint8_t *DataBuf,uint32_t dest_Page_address,uint16_t total_pages)
{
	uint32_t write_address=dest_Page_address;
	uint16_t i=0;
	
	for(i=0;i<total_pages;i++)
	{
		Flash_WriteMultiple(DataBuf,write_address,DataBuf,FLASH_PAGE_SIZE);
		write_address+=FLASH_PAGE_SIZE;		
	}
}
